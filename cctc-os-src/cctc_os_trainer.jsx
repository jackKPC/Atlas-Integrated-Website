...
import DATA from "./data/cctc-data.json";
import SLIDES from "./data/cctc-slides.json";
import { runCommand, initialSandboxState } from "./sandbox.js";

// ══ HOLO palette — light, futuristic (blue/yellow) — shared with the
//    CCNA3 trainer's engine, forked here for the CCTC lineup. ══
const C = {
...
// ══════════════════════════════════════════════════════════
//  GLOBAL STYLE — fonts already linked in cctc-os.html; here we inject
...
const TERMINAL_LINES = [
  ...
  "Get-ChildItem -Path C:\\labs\\cctc",
  "New-Item -Type File notes.txt",
...
// ══════════════════════════════════════════════════════════
//  DATA — CCTC lesson content. Only Module 1 (PowerShell) has real
//  study/drill/teach content right now; the rest are scaffolded as
//  locked placeholders until their lesson text is authored.
// ══════════════════════════════════════════════════════════
const ALL = [];
...
// ══════════════════════════════════════════════════════════
//  TOOLTIP ENGINE — every glossary term is hoverable everywhere it
//  appears (term chips, command rows, and inline mentions inside
//  facts/descriptions), showing a floating definition card. Hover to
//  preview, click/tap or focus to pin it open, Escape or an outside
//  click to unpin.
// ══════════════════════════════════════════════════════════
...
function CmdRow({ cmd, onTry }) {
  ...
  return (
    ...
    >
      ...
      {onTry && (
        <button
          onClick={(e) => { e.stopPropagation(); onTry(cmd.command); }}
          style={{ marginLeft: "auto", fontFamily: HEAD, fontWeight: 700, fontSize: 11.5, color: "#5ef2c0", background: C.dark, border: "1px solid rgba(94,242,192,.4)", borderRadius: 999, padding: "4px 10px", cursor: "pointer" }}
        >▶ Try it</button>
      )}
    </div>
  );
...
// ══════════════════════════════════════════════════════════
//  PROGRESS + MODE + SANDBOX PERSISTENCE — saved to this browser's
//  localStorage, namespaced separately from the CCNA3 trainer.
// ══════════════════════════════════════════════════════════
const PROGRESS_KEY = "cctc-os-progress-v1";
...
const SANDBOX_KEY = "cctc-os-sandbox-v1";
function loadSandboxState() {
  try {
    const raw = localStorage.getItem(SANDBOX_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" && parsed.fs ? parsed : null;
  } catch (e) { return null; }
}
function saveSandboxState(state) {
  try { localStorage.setItem(SANDBOX_KEY, JSON.stringify(state)); } catch (e) { /* storage unavailable */ }
}
...
// ══════════════════════════════════════════════════════════
const MODE_KEY = "cctc-os-mode-v1";
function loadMode() {
...
// ══════════════════════════════════════════════════════════
//  TERMINAL SANDBOX — a persistent, stateful, simulated PowerShell
//  5.1 session backed by cctc-os-src/sandbox.js. History and
//  filesystem state are saved to localStorage so a reload doesn't
//  wipe out a practice session.
// ══════════════════════════════════════════════════════════
function TerminalSandbox({ launchCmd, onLaunchConsumed }) {
  const [state, setState] = useState(() => loadSandboxState() || initialSandboxState());
  const [history, setHistory] = useState([]); // [{cwd, cmd, out:[], isError}]
  ...
  useEffect(() => { saveSandboxState(state); }, [state]);
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight; }, [history]);

  const exec = (raw) => {
    const cwdAtRun = stateRef.current.cwd;
    const r = runCommand(stateRef.current, raw);
    setState(r.state);
    setHistory((h) => [...h, { cwd: cwdAtRun, cmd: raw, out: r.lines, isError: r.isError, reference: /^\[Reference/.test(r.lines[0] || "") }]);
  };
  ...
  const submit = () => {
    const v = input;
    if (!v.trim()) return;
    if (v.trim().toLowerCase() === "clear" || v.trim().toLowerCase() === "cls") { setHistory([]); setInput(""); setHistIdx(-1); return; }
    exec(v);
    setInput("");
    setHistIdx(-1);
  };
  const onKeyDown = (e) => {
    ...
    if (e.key === "ArrowUp") {
      ...
      const idx = histIdx === -1 ? cmds.length - 1 : Math.max(0, histIdx - 1);
      setHistIdx(idx); setInput(cmds[idx]);
    }
    if (e.key === "ArrowDown") {
      e.preventDefault();
      if (histIdx === -1) return;
      const idx = histIdx + 1;
      if (idx >= cmds.length) { setHistIdx(-1); setInput(""); return; }
      setHistIdx(idx); setInput(cmds[idx]);
    }
  };
  const reset = () => { const s = initialSandboxState(); setState(s); setHistory([]); saveSandboxState(s); };

  return (
    <div style={{ background: "#0b0a17", border: "1px solid rgba(37,99,235,.4)", borderRadius: 14, overflow: "hidden", boxShadow: "0 14px 34px -18px rgba(76,29,149,.5)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", borderBottom: "1px solid rgba(255,255,255,.08)" }}>
        ...
        <span style={{ marginLeft: 8, fontFamily: MONO, fontSize: 11, color: "#8ba3d6" }}>PowerShell 5.1 — simulated sandbox</span>
        <button onClick={reset} style={{ marginLeft: "auto", fontFamily: HEAD, fontWeight: 700, fontSize: 11, color: "#8ba3d6", background: "transparent", border: "1px solid rgba(255,255,255,.18)", borderRadius: 999, padding: "4px 10px", cursor: "pointer" }}>↺ Reset</button>
      </div>
      <div ref={scrollRef} onClick={() => inputRef.current && inputRef.current.focus()} style={{ padding: 14, fontFamily: MONO, fontSize: 12.5, color: "#d7e3ff", height: 300, overflowY: "auto", lineHeight: 1.6, cursor: "text" }}>
        {history.length === 0 && (
          <div style={{ color: "#5a6a94", marginBottom: 8 }}>Type a command and press Enter — try <span style={{ color: "#5ef2c0" }}>Get-Process</span> or click "▶ Try it" on any command above.</div>
        )}
        {history.map((h, i) => (
          <div key={i} style={{ marginBottom: 6 }}>
            <div><span style={{ color: "#5ef2c0" }}>PS {h.cwd}&gt;</span> <span style={{ color: "#fff" }}>{h.cmd}</span></div>
            {h.out.map((l, j) => (
              <pre key={j} style={{ margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-word", color: h.isError ? "#ff8383" : h.reference ? "#eab308" : "#d7e3ff", fontFamily: MONO }}>{l}</pre>
            ))}
          </div>
        ))}
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <span style={{ color: "#5ef2c0", flexShrink: 0 }}>PS {state.cwd}&gt;</span>
          <input
            ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={onKeyDown}
            spellCheck={false} autoComplete="off" autoCapitalize="off"
            style={{ flex: 1, minWidth: 40, background: "transparent", border: "none", outline: "none", color: "#fff", fontFamily: MONO, fontSize: 12.5 }}
          />
        </div>
      </div>
    </div>
...
//  consistently in the site's own palette.
// ══════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
const BOOT_LINES = [
  "INIT CCTC-OS TRAINER",
  "MOUNTING VIRTUAL FILESYSTEM /labs/cctc",
  "SEEDING PROCESS + SERVICE TABLES",
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", textAlign: "center", padding: "0 20px" }}>
        <div style={{ fontFamily: DISPLAY, fontWeight: 900, fontSize: "clamp(30px,6.5vw,50px)", letterSpacing: 3, ...gradText, opacity: 0, animation: "boot-fade-up .8s ease .6s forwards" }}>CCTC-OS</div>
        <div style={{ fontFamily: HEAD, fontWeight: 600, fontSize: 12.5, letterSpacing: 4, color: "#8ba3d6", marginTop: 6, opacity: 0, animation: "boot-fade-up .8s ease .9s forwards" }}>CYBER COMMON TECHNICAL CORE</div>
        ...
        <div style={{ marginTop: 22, width: "min(70vw, 280px)", height: 3, background: "rgba(255,255,255,.1)", borderRadius: 999, overflow: "hidden", marginInline: "auto" }}>
          <div style={{ height: "100%", background: "linear-gradient(90deg,#2563eb,#06b6d4,#eab308)", backgroundSize: "200% 100%", animation: "gradient-flow 2s ease infinite, boot-progress 3.6s cubic-bezier(.2,.8,.2,1) forwards" }} />
        </div>
      </div>
...
//  ENGINE + APP
// ══════════════════════════════════════════════════════════
export default function CCTCTrainer() {
  useGlobalStyle();
  const mode = useMode();
  const simple = mode === "simple";
  ...
  const tryCommand = (cmdText) => {
    setLaunchCmd(cmdText);
    if (terminalRef.current) terminalRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  ...
  const drillContextDesc = () => {
    ...
    return "Focus your teaching on the concept this question tests. Current practice item: " + qDesc;
  };
  ...
  const callLLM = async (contextDesc, userMsg, isFirstBreakdown) => {
    const ctx = "You are a patient instructor for the Cyber Common Technical Core (CCTC), a foundational course for military and DoD cyber operators covering PowerShell, Windows/Linux internals, auditing, memory analysis, and Active Directory enumeration. " + contextDesc +
  ...
  const mastery = (mi) => { const qs = DATA[mi].questions || []; if (!qs.length) return 0; let s = 0; qs.forEach((q) => s += getP(q.key).box); return Math.round((s / (3 * qs.length)) * 100); };
  ...
  if (view === "home") {
    return (
      <div style={page}>
        ...
        <ModeToggle />
        <div style={wrap}>
        <div style={{ fontFamily: MONO, fontSize: 12, color: C.dim }}>CCTC · Cyber Common Technical Core · 14 modules · {ALL.length} drill items</div>
        <div style={{ position: "relative", overflow: "hidden", display: "inline-block" }}>
          <h1 style={{ fontSize: 40, margin: "8px 0 4px", lineHeight: 1.05, ...(simple ? gradTextFlat : gradText) }}>CCTC-OS</h1>
        ...
        <p style={{ color: C.dim, fontSize: 14.5, marginTop: 4, lineHeight: 1.6, maxWidth: 620 }}>Fast-track knowledge transfer with real drilling, real commands, and a hands-on simulated terminal you can practice in as you go. Ask the tutor anything after you answer.</p>
        ...
        {DATA.map((m, mi) => {
          const pct = m.locked ? 0 : mastery(mi);
          return (
            <Reveal key={mi} delay={Math.min(mi, 9) * 45}>
            <div style={simple
              ? { background: "#fff", border: "1px solid #dbe4f0", borderRadius: 10, padding: 14, marginBottom: 10, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", opacity: m.locked ? .65 : 1 }
              : { background: C.panel, backdropFilter: "blur(10px)", border: "1px solid " + C.line, borderRadius: 14, padding: 14, marginBottom: 10, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", boxShadow: "0 6px 20px -14px rgba(76,29,149,.25)", opacity: m.locked ? .65 : 1 }}>
              <div style={{ fontFamily: DISPLAY, fontWeight: 900, fontSize: 12, color: C.violet, minWidth: 74 }}>MOD {String(m.num).padStart(2, "0")}</div>
              <div style={{ fontWeight: 700, fontFamily: HEAD, fontSize: 16, flex: 1, minWidth: 200 }}>
                {m.title}{" "}
                {m.locked
                  ? <span style={chip(C.dim)}>🔒 coming soon</span>
                  : <span style={{ color: C.dim, fontWeight: 500, fontSize: 12.5, fontFamily: BODY }}>· {m.questions.length} q</span>}
              </div>
              {!m.locked && (
                <>
              ...
              {m.locked && <span style={{ fontFamily: MONO, fontSize: 11, color: C.dim }}>Lesson {m.lessonCode}</span>}
            </div>
            </Reveal>
          );
        })}
  ...
  if (view === "study") {
    ...
    return (
      ...
      <div style={page}>
        ...
        {m.commands.length > 0 && (
          <div style={panel}>
            <div style={secLabel(C.cyan)}>Commands — cheat sheet <span style={{ textTransform: "none", color: C.dim, fontWeight: 500, letterSpacing: 0 }}>(hover for details, or try it live below)</span></div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {m.commands.map((cmd, i) => <CmdRow key={i} cmd={cmd} onTry={tryCommand} />)}
            </div>
          </div>
        )}
        ...
        <div ref={terminalRef} style={panel}>
          <div style={secLabel(C.magenta)}>Practice terminal <span style={{ textTransform: "none", color: C.dim, fontWeight: 500, letterSpacing: 0 }}>(simulated PowerShell — state persists on this device)</span></div>
          <TerminalSandbox launchCmd={launchCmd} onLaunchConsumed={() => setLaunchCmd(null)} />
        </div>
